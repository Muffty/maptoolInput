...
public class ClientMethodHandler extends AbstractMethodHandler {
	...
	public void handleMethod(final String id, final String method, final Object... parameters) {
		...
		if(cmd == COMMAND.putToken){
            GUID map = (GUID)parameters[0];
            Token token = (Token)parameters[1];

            if(!MyData.instance.hasToken(map, token)){
                MyData.instance.addToken(map, token, token.x, token.y);
            }else{
                MyData.instance.updateToken(map, token);
            }
            ...
            MyData.instance.updateTokenByID(map, token, x, y);

        }else if(cmd == COMMAND.enforceZone){
            MyData.instance.currentMap = (GUID)parameters[0];
            if(Connector.currentConnection != null)
                Connector.currentConnection.NotifyMapLoaded(MyData.instance.currentMap);
        }
